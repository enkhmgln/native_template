...
function ToastView({ toast, onHide }: { toast: ToastItem; onHide: () => void }) {
  ...
  }, [dismiss, opacity, scale, toast.id, translateY]);
  ...
  return (
    <View
      ...
      style={[styles.host, { paddingHorizontal: spacing.lg, top: insets.top + spacing.sm }]}
    >
      <Animated.View
      ...
      >
        <Ionicons color={feedback.tint} name={feedback.icon} size={22} style={styles.icon} />
...
export function Toast() {
  ...
  return <ToastView key={toast.id} toast={toast} onHide={hide} />;
}
